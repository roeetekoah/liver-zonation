...
Reads:
  ../data/combined_scRNAseq_atlas_M5M6M7M8.mat   (Paper 2 snRNA atlas, v7.3 HDF5)
  pericentral_genes.txt, periportal_genes.txt    (signatures)
  paper1/genes.txt                               (to keep only features shared with Paper 1)
Writes:
  paper2_train.npz = { X (cells x feats, float32), feats (str[]),
...
Run from the Hackathon folder:   python analysis/convert_paper2_mat.py
...
P1G  = set(g.strip() for g in open(os.path.join(HERE, "paper1", "genes.txt")))
OUT  = os.path.join(HERE, "paper2_train.npz")
