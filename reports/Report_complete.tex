...
\vspace{0.45em}
\noindent\textbf{Roee Tekoah}~(\href{mailto:roee.tekoah@mail.huji.ac.il}{roee.tekoah@mail.huji.ac.il})\par
\vspace{0.12em}
...
\subsection{The one biopsy-internal program that does change: the pericentral detox program dims}\label{sec:dimming}
...
\paragraph{What this is, and what it is not.} This is \textbf{dimming}: the pericentral cells keep their \emph{class} (the anchor fractions in Sec.\ 3.3 stay flat), but the \emph{level} of their detox program declines. The robust decline is the cytochrome-P450 xenobiotic core --- the only set that clears the self-contained \texttt{mroast} test on its own (FDR 0.015) and is reproduced by the per-cell measure. The broader pericentral landmark set and the phase-II set lean down only faintly: they reach the competitive bar but \emph{fail} the self-contained test (roast FDR 0.081 and 0.220), no single gene is individually significant, and the landmark set largely overlaps the detox genes --- a shift this weak cannot be called a turn-off, and it is not independent evidence. What keeps the classification intact is the two \emph{gating} anchors used to call a nucleus pericentral, GLUL and CYP3A4, which are individually flat (per-gene F4-vs-F1 FDR 0.80 and 0.85; a binary $\ge$2-UMI detection call is robust to a modest level drop), so the pericentral-anchor fraction stays flat while the program's level falls around them. Three caveats are load-bearing: the decline is \textbf{relative} --- detox transcripts within a fixed 1{,}500-UMI budget, \emph{not} proven absolute molecule loss; it is a \textbf{donor-level trend}, not pseudoreplicated; and it \textbf{could partly reflect a shift among pericentral subzones} (fewer deep zone-3 high-detox cells) rather than pure within-cell downregulation. It is \textbf{functional dimming of a zonated program, not de-zonation or transdifferentiation}, and it does not rescue the paper's identity claim. By analogy: the radio station did not change genre (no de-zonation); the volume dropped (the program dims).

\begin{figure}[H]\centering\includegraphics[width=0.95\linewidth]{fig_dimming}\caption{The pericentral program dims. \emph{Left:} within-PC detox output --- detox transcripts per pericentral nucleus, depth-matched to 1{,}500 UMIs --- falls from $\sim$11.9 (F1) to $\sim$8.8 (F4); donor-level Spearman $\rho=-0.48$, $p=0.003$ (F0 faded, n$=$2). \emph{Right:} pre-specified gene-set programs (\texttt{camera}, signed $-\log_{10}$FDR): the pericentral CYP detox set down and robust (the only set also clearing the self-contained test); the pericentral landmark and phase-II sets down only competitively (faint); biliary/ductular and the fibrogenesis/inflammation controls up, ER-stress flat. The left-panel detox output is measured on detox genes (CYP2E1, CYP1A2, ADH4\ldots); the gating anchors GLUL/CYP3A4 are individually flat (FDR 0.80, 0.85), so the anchor fraction stays flat while the program's level falls.}\label{fig:dimming}\end{figure}
...
Four results, at decreasing confidence levels. \textbf{Main result:} across acquisition-matched needle-biopsy MASLD (F1$\to$cirrhotic F4), there is \textbf{no transcriptional de-zonation signal} --- cells keep their zonal class, the anchor classification is flat across the full sensitivity grid, and an equivalence test excludes a $\sim$20-point coordinated shift (interior $\sim$12 points), while not excluding a $\le$10-point drift. The dramatic de-zonation in the pooled analysis is largely a tissue-acquisition confound: the signal lives in deceased-donor organ tissue carrying organ-wide procurement stress, and within the explants it is heterogeneous rather than one program. \textbf{The one confirmed biopsy-internal change:} the pericentral \emph{detox} program dims in level across fibrosis. The robust part is the CYP xenobiotic core --- significant in both a competitive (camera FDR $2\times10^{-6}$) and a self-contained (roast FDR 0.015) gene-set test, plus a per-cell, depth-matched decline (within-PC detox output $\sim$11.9$\to$8.8 transcripts/nucleus F1$\to$F4, Spearman $\rho=-0.48$, $p=0.003$) that survives TMM and contaminant removal. The broader landmark and phase-II sets lean down only faintly (competitive-significant but not self-contained; no single gene significant). The cells keep their pericentral class (the gating anchors GLUL/CYP3A4 stay individually flat, FDR 0.80/0.85). This is a \emph{relative} decline, not proven absolute loss, and is functional dimming, not de-zonation. \textbf{Secondary result:} the one per-gene genome-wide change is a biliary/ductular marker rise (64 hits) inside hepatocyte-labeled pseudobulk; zonation genes are flat (GLUL FDR 0.80). \textbf{Interpretive lead:} that biliary rise is most consistent with cholangiocyte ambient RNA plus rare mixed/doublet nuclei, with CXCL10 a separate candidate inflammatory signal --- a promising but unclosed source attribution.
...
All entries web-verified against the primary sources (authors, title, journal, year, volume/pages).

\begin{enumerate}[leftmargin=1.6em]
  \item Gribben C, Galanakis V, Calderwood C, et al.\ \emph{Acquisition of epithelial plasticity in human chronic liver disease.} \textbf{Nature} 630:166--173, 2024. doi:10.1038/s41586-024-07465-2. (GSE202379.) --- verified against the PDF in \texttt{papers/}.
  \item Squair JW, Gautier M, Kathe C, et al.\ \emph{Confronting false discoveries in single-cell differential expression.} \textbf{Nature Communications} 12:5692, 2021. --- pseudobulk vs cell-level false positives.
  ...
  \item Leek JT, Scharpf RB, Corrada Bravo H, et al.\ \emph{Tackling the widespread and critical impact of batch effects in high-throughput data.} \textbf{Nature Reviews Genetics} 11:733--739, 2010. --- batch/biology confounding.
