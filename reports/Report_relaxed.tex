...
This is \textbf{dimming}, not de-zonation. Like a radio: the station did not change genre (the cells keep their pericentral identity --- the anchor fractions in R2 stay flat), but the volume dropped (the detox program's level falls). It works precisely because the identity anchors (GLUL, CYP3A4 --- a yes/no detection call) are \emph{different genes} from the detox-module genes measured for dimming (CYP2E1, CYP1A2, ADH4\ldots), so the anchor fraction can stay flat while the module quiets. Three honest caveats: this is a \textbf{relative} decline (detox transcripts within a fixed sequencing budget, not proven absolute molecule loss); it is a donor-level trend; and it could partly reflect a shift among pericentral subzones rather than pure within-cell downregulation. It does \emph{not} rescue the paper's identity-loss claim.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{fig_dimming}
\caption{The pericentral detox module dims. \emph{Left:} detox transcripts per pericentral nucleus (depth-matched to 1{,}500 UMIs) fall from $\sim$11.9 (F1) to $\sim$8.8 (F4); donor-level Spearman $\rho=-0.48$, $p=0.003$. \emph{Right:} pre-specified gene-set programs --- pericentral detox and identity down, biliary and fibrogenesis/inflammation controls up, ER-stress flat. Measured on detox genes disjoint from the GLUL/CYP3A4 identity anchors, so the anchor fraction stays flat while the module dims.}
\label{fig:dimming}
\end{figure}
