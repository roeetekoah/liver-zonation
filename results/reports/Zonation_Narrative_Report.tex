...
\hline
unsupervised\_combined & 4769 & 3719 & -0.50 & 0.72 & -0.51 & control (Paper1-fit) \\
unsupervised\_p2 & 5177 & 3302 & -0.47 & 0.69 & -0.50 & PRIMARY (label-free) \\
expanded\_curated & 26 & 23 & -0.48 & 0.67 & -0.43 & PRIMARY (interpretable) \\
...
(\texttt{expanded\_curated}) and a \textbf{label-free} learned axis (\texttt{unsupervised\_p2}, trained entirely on the external
Paper~2 healthy atlas). On healthy quality they are statistically indistinguishable
(healthy-quality scores 1.15 vs 1.16, a gap of 0.012 --- within the 0.02 tie band), so picking one over the other would be arbitrary --- and the label-free axis is in
...
tiny curated \texttt{core\_curated} set collapses harder) --- choosing by disease strength would be
...
PCA axis $\rho=-0.504$ (perm $p=0.0005$). The broken ``full'' ruler is null ($\rho=-0.042$),
...
\begin{center}\includegraphics[width=\textwidth]{C:/liver-zonation/results/figures/ruler_panel.png}\end{center}
