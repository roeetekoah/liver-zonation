...
"""
import os, sys, time, numpy as np, h5py, matplotlib
matplotlib.use("Agg")
...
HERE = os.path.dirname(os.path.abspath(__file__))     # .../Hackathon/src
MAT  = str(config.DATA_RAW / "combined_scRNAseq_atlas_M5M6M7M8.mat")  # atlas file: has t/gene_name, leiden, cluster_unique
PCf  = str(config.PC_GENES)
PPf  = str(config.PP_GENES)
...
plt.tight_layout(); out = os.path.join(str(config.FIGURES), "p2_validation.png"); plt.savefig(out, dpi=130)
print("saved", out)
