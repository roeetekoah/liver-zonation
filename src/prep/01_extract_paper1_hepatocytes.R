...
# Run from the repo root:   Rscript src/prep/01_extract_paper1_hepatocytes.R
suppressMessages({ library(Seurat); library(Matrix) })

# resolve repo root from this script's location (src/prep/ -> ../../)
.args <- commandArgs(trailingOnly = FALSE)
.sf <- sub("^--file=", "", .args[grep("^--file=", .args)])
SCRIPT_DIR <- if (length(.sf)) dirname(normalizePath(.sf)) else getwd()
ROOT     <- normalizePath(file.path(SCRIPT_DIR, "..", ".."))
RAW      <- file.path(ROOT, "data", "raw")
PROC_P1  <- file.path(ROOT, "data", "processed", "paper1")
RESULTS  <- file.path(ROOT, "results")

# find the actual .rds FILE (the .gz may have extracted into a same-named folder)
rdsf <- list.files(RAW, pattern = "GSE202379.*\\.rds$", recursive = TRUE, full.names = TRUE)
...
dir.create(PROC_P1, recursive = TRUE, showWarnings = FALSE)
dir.create(RESULTS, recursive = TRUE, showWarnings = FALSE)

# ---- preview every categorical column so we can always see the structure ----
sink(file.path(RESULTS, "meta_preview.txt"))
...
if (is.na(ct_col)) stop("No column has a 'hepato...' label; inspect results/meta_preview.txt and tell Claude.")
...
Matrix::writeMM(counts, file.path(PROC_P1, "counts.mtx"))
writeLines(rownames(counts), file.path(PROC_P1, "genes.txt"))
writeLines(colnames(counts), file.path(PROC_P1, "barcodes.txt"))

hm <- hep@meta.data
out <- data.frame(cell_id = rownames(hm),
                  cell_type = as.character(hm[[ct_col]]),
                  stage = as.character(hm[[stage_col]]))
write.csv(out, file.path(PROC_P1, "cell_metadata.csv"), row.names = FALSE)
write.csv(cbind(cell_id = rownames(md), md), file.path(PROC_P1, "metadata_all_cells.csv"), row.names = FALSE)
cat("\nWrote data/processed/paper1/{counts.mtx, genes.txt, barcodes.txt, cell_metadata.csv, metadata_all_cells.csv}\n")
cat("Check results/meta_preview.txt to confirm the auto-detected columns look right.\n")
