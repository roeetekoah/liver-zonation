...
\code{core}            & $13 / 8$ & hand-curated canonical markers \\
\code{paper2\_landmark} & $20 / 20$ & EXACT Paper 2 hepatocyte landmark genes (verbatim, \code{Hepatocyte-*-LM.csv}) \\
\bottomrule
\end{tabular}
\end{center}
...
\code{paper2\_landmark} is the \emph{credibility control}: the exact 20$+$20 hepatocyte landmark genes
Paper 2 itself uses to assign zonation (verbatim from \code{Hepatocyte-*-LM.csv}), canonical and fully
interpretable. If it shows the \emph{same} collapse, the signal is not an artefact of
...
\subsection{Normalization (used by scoring; the \code{zrows()} helper)}\label{sec:norm}
...
style). $Y$ is the z-scored $\log 1p$-CP10k expression: for gene $g$, cell $c$ with raw count
$x_{gc}$ and library size $\ell_c=\sum_{g'} x_{g'c}$,
\[
  v_{gc}=\log\!\Big(1+10^{4}\,\tfrac{x_{gc}}{\ell_c}\Big),
  \qquad
  Y_{gc}=\frac{v_{gc}-\operatorname{mean}_{c'} v_{gc'}}{\operatorname{std}_{c'} v_{gc'}}
\]
(CP10k $\to \log 1p \to$ per-gene z-score across cells; the same normalization as Step~2,
\S\ref{sec:norm}). Then for cell $c$,
\[
...
Then, with each arm standardised to unit variance across cells (so the PC/PP gene-count
imbalance does not tilt the axis; Section~\ref{sec:sigsets}):
\begin{align}
  \text{pc} &= \text{score(PERICENTRAL)}, & \text{pp} &= \text{score(PERIPORTAL)}, \\
  \coordc   &= \text{pc} - \text{pp}\ \ (>0\text{ pericentral}), & \text{plast} &= \text{score(PLASTICITY)}.
\end{align}
The plasticity set is \code{signatures/plasticity.txt} (\code{KRT7, KRT19, SOX9, SOX4, KRT23,
NCAM1}). \textbf{Run this for every set in \code{config.SETS\_TO\_COMPARE}} --- the coordinate is
produced for \code{full} \emph{and} \code{paper2\_landmark}, so Step~5 can pick the leader from the
data. The default gene set is the \code{full} program (Section~\ref{sec:sigsets}). The simpler
...
\paragraph{Run the gate for \emph{both} sets --- and let it pick the leader.} Compute Step~5 (and
the Step~5b ruler battery) for \code{full} \emph{and} \code{paper2\_landmark}. This is where the
``\code{full} vs landmark'' choice is actually settled, \emph{by evidence rather than by assertion}:
lead with whichever set validates more cleanly on the healthy positive control (marker $\rho$ signs
$+$ magnitudes, low entropy, strong PC--PP anti-correlation, high split-half reproducibility), and
report the other alongside as the control. Honest expectation: \code{full} usually wins on resolution
and is required for H2's held-out test; \code{paper2\_landmark} may transfer more cleanly across the
snRNA$\rightarrow$Visium platform gap (fewer, canonical, high-expression genes carry less
cross-dataset technical variance). If the two \emph{disagree} on the collapse, that disagreement is a
finding, not a nuisance --- it flags the ruler as disease/platform-sensitive and must be reported.
