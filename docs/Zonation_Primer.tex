...
$q<0.05$; the default) \emph{and} the exact Paper~2 hepatocyte landmark set (20$+$20); plus \code{expanded}, \code{core},
...
\aside{The signature, run both ways --- \code{full} is the result, landmark is the audit}{We compute
the coordinate with \textbf{two} gene schemes and report both --- but they do \emph{different jobs}, not
a 50/50 hedge.
\textbf{(a)~The full transcriptome-wide program is the primary instrument} --- the $\sim$1{,}640
significantly-zonated hepatocyte genes ($q<0.05$, well-expressed, present in Paper~1; PC/PP $=$
1{,}273\,/\,364), the \textbf{default}. It is primary for three reasons: it \emph{is} the contribution
(read position from the entire program, not 2--3 markers by eye); it is the \emph{more robust} ruler in
disease --- because the coordinate is a \emph{mean} of z-scored genes, disease-corruption of any one
marker is diluted (e.g.\ \gene{CYP2E1}, canonical pericentral, is induced in NASH for non-spatial
reasons and could swing a 13-gene ruler, but is 1-of-$\sim$1{,}600 here); and only a large set makes the
non-circular held-out-gene test (\S\ref{sec:workplan}, Step~7) and the ``which programs collapse'' (H2)
question meaningful.
\textbf{(b)~The landmark set (20$+$20) is the credibility control} --- the \emph{exact} hepatocyte
landmark genes Paper~2 uses to assign zonation (verbatim from its \code{Hepatocyte-PC-LM.csv} /
\code{Hepatocyte-PP-LM.csv}; these define $\eta = \text{sum}_{pp}/(\text{sum}_{pp}+\text{sum}_{pc})$).
Its job is to rebut ``did you
just engineer this with enough genes?'': if a maximally-different small ruler shows the \emph{same}
collapse, the signal is not a gene-set artefact; if the two \emph{disagree}, that is itself a finding (a
warning the ruler may be disease-sensitive). It is also fully interpretable. Two further sets are an
optional robustness gradient --- a $\sim$100-gene \code{expanded} (top-ranked $\cup$ landmark $\cup$ core)
and a hand-curated 13/8 \code{core}.
\textbf{Why the original plan said ``landmark.''} Marker-based reconstruction is the field-standard,
validated method --- exactly what Paper~2 and the foundational liver-zonation work (Halpern et al.\ 2017)
did. Landmark was the conservative starting point; \code{full} is the deliberate upgrade, not a
correction of a mistake. (An earlier draft did briefly make landmark the \emph{default}, which silently
contradicted the transcriptome-wide motivation; corrected --- \code{full} is the default now.)
\textbf{On the PC/PP imbalance (1{,}273 vs 364).} This is a \emph{feature}-set asymmetry, not class
imbalance, and it does \emph{not} bias the coordinate: each arm is a \emph{mean} of z-scores, so the
3.5:1 count ratio carries no magnitude bias. It only makes the periportal arm noisier (fewer genes
averaged); we neutralise that by standardising each arm to unit variance before subtracting, and verify
with a size-matched (top-364 PC) run.}
...
$\sim$1{,}640-gene set, not $\sim$20 landmarks), and we exclude/flag signature genes from any ``driver'' claim.
