...
question meaningful.
\textbf{(b)~The exact Paper~2 landmark set (13$+$8) is the credibility control} --- the verbatim,
canonical, biologically-named genes Paper~2 used to seed its axis. Its job is to rebut ``did you just
engineer this with enough genes?'': if a maximally-different small ruler shows the \emph{same} collapse,
the signal is not a gene-set artefact; if the two \emph{disagree}, that is itself a finding (a warning
the ruler may be disease-sensitive). It is also faithful and fully interpretable. Two further sets are an
optional robustness gradient --- a $\sim$100-gene \code{expanded} (landmark $\cup$ core $\cup$ top-ranked)
and a curated 13/8 \code{core}.
